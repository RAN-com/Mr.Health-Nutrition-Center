...
const Faqitems = ({item}) => {
  ...
  const toggleAccordion =()=>{
   ...
   <div className={`${isOpen && "bg-blue-300 text-white border-none" } w-7 h-7 lg:w-8 lg:h-8 border border-solid border-[#FFF] rounded flex items-center justify-center`}>
    {isOpen ? <AiOutlineMinus/>:<AiOutlinePlus/>}
   </div>
        </div>
        {isOpen && <div className='mt-4'>
            <p className='text-[14px] leading-6 lg:text-[16px] lg:leading-7 font-bold  text-white '>{item.content}</p>
            </div>}
    </div>
